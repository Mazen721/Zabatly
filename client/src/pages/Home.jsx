...
function ImageCarousel({ images, alt, t }) {
  ...
  return (
    <div className="relative aspect-[4/3] overflow-hidden bg-sand-100 group/img">
      {src ? (
        ...
        <div className="w-full h-full flex items-center justify-center text-sand-400 text-sm">{t('common:vehicle.photoUnavailable')}</div>
      )}
      {multi && (
        <>
          <button onClick={prev} aria-label={t('previousImage')} className="absolute left-2 rtl:left-auto rtl:right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-sand-50/90 flex items-center justify-center text-sand-700 opacity-0 group-hover/img:opacity-100 transition-opacity duration-200 z-10">
            <svg className="w-3.5 h-3.5 rtl:scale-x-[-1]" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M10 3L5 8l5 5" /></svg>
          </button>
          <button onClick={next} aria-label={t('nextImage')} className="absolute right-2 rtl:right-auto rtl:left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-sand-50/90 flex items-center justify-center text-sand-700 opacity-0 group-hover/img:opacity-100 transition-opacity duration-200 z-10">
            <svg className="w-3.5 h-3.5 rtl:scale-x-[-1]" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M6 3l5 5-5 5" /></svg>
          </button>
...
function SaveButton({ saved, onToggle, t }) {
  return (
    <button
      type="button"
      onClick={onToggle}
      ...
      className={`absolute bottom-2.5 right-2.5 rtl:right-auto rtl:left-2.5 z-30 flex h-8 w-8 items-center justify-center rounded-full border transition-colors duration-150 ${
        saved
...
function VehicleCard({ vehicle, recommended, saved, onToggleSave, t, tc }) {
  return (
    <Link
    ...
    >
      <div className="relative">
        <ImageCarousel images={vehicle.images} alt={`${vehicle.make} ${vehicle.model}`} t={t} />
        <SaveButton saved={saved} onToggle={onToggleSave} t={t} />
        <span className="absolute top-2.5 left-2.5 rtl:left-auto rtl:right-2.5 bg-sand-50/90 backdrop-blur-sm px-2 py-0.5 rounded text-[0.6875rem] font-semibold text-sand-700 capitalize z-10">
          {vehicle.type}
        </span>
        {recommended && (
          <span className="absolute top-2.5 right-2.5 rtl:right-auto rtl:left-2.5 bg-signal-500 text-primary-950 px-2 py-0.5 rounded text-[0.6875rem] font-bold z-10">
            {tc('vehicle.recommended')}
          </span>
        )}
        {!vehicle.isAvailable && (
          <div className="absolute inset-0 bg-sand-50/70 backdrop-blur-[2px] z-20 flex items-center justify-center">
            <span className="bg-primary-800 text-white px-4 py-1.5 rounded-subtle text-xs font-bold">{tc('vehicle.rented')}</span>
          </div>
        )}
      </div>
      ...
      <div className="p-4">
        <div className="flex justify-between items-start gap-2 mb-2">
          ...
          <div className="text-right rtl:text-left shrink-0">
            ...
            <span className="text-[0.7rem] text-sand-500 ml-0.5 rtl:ml-0 rtl:mr-0.5">{tc('vehicle.egpDay')}</span>
          </div>
        </div>

        <div className="flex gap-1.5 flex-wrap mb-3">
          <span className="text-[0.7rem] text-sand-600 bg-sand-100 px-1.5 py-0.5 rounded">{vehicle.transmission === 'automatic' ? tc('vehicle.auto') : tc('vehicle.manual')}</span>
          <span className="text-[0.7rem] text-sand-600 bg-sand-100 px-1.5 py-0.5 rounded">{tc('vehicle.seats', { count: vehicle.capacity })}</span>
          {vehicle.has_driver && (
            <span className="text-[0.7rem] text-primary-700 bg-primary-50 px-1.5 py-0.5 rounded font-medium">{tc('vehicle.driver')}</span>
          )}
        </div>
    ...
    <div className="space-y-1">
      {options.map((opt) => (
        <button
          ...
          className={`w-full text-left rtl:text-right px-3 py-1.5 rounded-subtle text-[0.8rem] transition-colors duration-100 ${
            opt.value === value
...
function CitySelect({ listed, value, onChange, t }) {
  return (
    <select
      value={value}
...
function PriceSlider({ value, onChange, t }) {
  const [min, max] = value;
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder={t('min')}
        />
        <span className="text-sand-400 text-[0.75rem] shrink-0">{t('to')}</span>
        <input
          type="number"
          min={min}
          max={PRICE_MAX}
...
export default function Home() {
  ...
  const filterContent = (
    <>
      ...
      <div className="mb-5">
        <div className="relative">
          <svg className="absolute left-3 rtl:left-auto rtl:right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-sand-400 pointer-events-none" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
          ...
          <input
            ...
            className="w-full bg-sand-100 border border-sand-200 text-sand-950 placeholder-sand-400 rounded-subtle pl-9 pr-3 rtl:pl-3 rtl:pr-9 py-2 text-[0.8rem] focus:outline-none focus:border-primary-600 focus:ring-1 focus:ring-primary-600 transition-colors"
          />
        </div>
      </div>
      ...
      <FilterSection title={t('priceRange')}>
        <PriceSlider value={priceRange} onChange={setPriceRange} t={t} />
      </FilterSection>
    ...
    <div className="min-h-screen bg-sand-50">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-10 pt-6 pb-16">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-[1.5rem] font-bold text-sand-950 leading-tight">{t('title')}</h1>
            {!loading && !error && (
              <p className="text-[0.8rem] text-sand-500 mt-1">{t('vehicleCount', { count: filtered.length })}</p>
            )}
          </div>
          ...
          >
            ...
            {t('filters')}{activeCount > 0 && <span className="bg-primary-800 text-white text-[0.6rem] font-bold rounded-full w-4 h-4 flex items-center justify-center">{activeCount}</span>}
          </button>
        </div>
        ...
        <div className="flex gap-8">
          ...
          {sidebarOpen && (
            <div className="fixed inset-0 z-50 lg:hidden">
              ...
              <div className="absolute left-0 rtl:left-auto rtl:right-0 top-0 bottom-0 w-72 bg-sand-50 border-r rtl:border-r-0 rtl:border-l border-sand-200 p-5 overflow-y-auto">
                <div className="flex items-center justify-between mb-5">
                  <span className="text-[0.9rem] font-semibold text-sand-950">{t('filters')}</span>
          ...
          <div className="flex-1 min-w-0">
            ...
            {error && (
              <div className="text-center py-20">
                ...
                <button onClick={() => window.location.reload()} className="text-primary-700 font-semibold hover:text-primary-800 transition-colors text-[0.8rem]">{tc('actions.retry')}</button>
              </div>
            )}
            ...
            {!loading && !error && filtered.length === 0 && (
              <div className="text-center py-20">
                <p className="text-[0.95rem] font-semibold text-sand-700 mb-2">{t('noMatch')}</p>
                <p className="text-[0.85rem] text-sand-400 mb-4">{t('widenSearch')}</p>
                <button onClick={clearAll} className="text-primary-700 font-semibold hover:text-primary-800 transition-colors text-[0.8rem]">{tc('actions.clearFilters')}</button>
              </div>
            )}
