...
export default function UserPublicProfile() {
    ...
    return (
      <div className="min-h-screen bg-sand-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-[0.95rem] font-semibold text-sand-700 mb-2">User not found.</p>
  ...
  const roleLabel = profile.role === 'agency' ? 'Vehicle Owner' : profile.role === 'driver' ? 'Freelance Driver' : 'Renter';
        ...
        <div className="flex items-center gap-5 mb-6">
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-1">
              ...
              {profile.is_verified && (
                <span className="text-[0.65rem] font-bold text-green-700 bg-green-50 px-1.5 py-0.5 rounded border border-green-200">Verified</span>
              )}
            </div>
          ...
          {!isRenter && (
            <>
              <div className="flex-1 bg-sand-50 px-4 py-3.5">
                <p className="text-[0.7rem] font-semibold uppercase tracking-[0.04em] text-sand-500 mb-0.5">Rating</p>
                <p className="text-[1.35rem] font-bold text-sand-900 tabular-nums">{profile.rating || 'New'}</p>
              </div>
              <div className="flex-1 bg-sand-50 px-4 py-3.5">
                <p className="text-[0.7rem] font-semibold uppercase tracking-[0.04em] text-sand-500 mb-0.5">Reviews</p>
                <p className="text-[1.35rem] font-bold text-sand-900 tabular-nums">{profile.numReviews || 0}</p>
              </div>
            </>
          )}
          <div className="flex-1 bg-sand-50 px-4 py-3.5">
            <p className="text-[0.7rem] font-semibold uppercase tracking-[0.04em] text-sand-500 mb-0.5">{isRenter ? 'Total Trips' : 'Total Rentals'}</p>
            <p className="text-[1.35rem] font-bold text-sand-900 tabular-nums">{profile.totalRentals || 0}</p>
          </div>
          {profile.vehicles?.length > 0 && (
            <div className="flex-1 bg-sand-50 px-4 py-3.5">
              <p className="text-[0.7rem] font-semibold uppercase tracking-[0.04em] text-sand-500 mb-0.5">Listed Cars</p>
            ...
            <div className="grid gap-px bg-sand-200 sm:grid-cols-3">
              {birthDate && (
                <div className="bg-sand-50 px-4 py-3.5">
                  <p className="text-[0.7rem] font-semibold uppercase tracking-[0.04em] text-sand-500 mb-0.5">Date of birth</p>
                  <p className="text-[0.875rem] font-semibold text-sand-900">{birthDate}</p>
                </div>
              )}
              {profile.phone && (
                <div className="bg-sand-50 px-4 py-3.5">
                  <p className="text-[0.7rem] font-semibold uppercase tracking-[0.04em] text-sand-500 mb-0.5">Phone</p>
                  <p className="text-[0.875rem] font-semibold text-sand-900 tabular-nums">{profile.phone}</p>
                </div>
              )}
              {profile.emergencyContact?.phone && (
                <div className="bg-sand-50 px-4 py-3.5">
                  <p className="text-[0.7rem] font-semibold uppercase tracking-[0.04em] text-sand-500 mb-0.5">Emergency</p>
                ...
                >
                  ...
                  <div className="p-3.5">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-[0.85rem] font-bold text-primary-800">{v.price_per_day} EGP<span className="text-sand-500 font-normal text-[0.75rem]"> / day</span></span>
